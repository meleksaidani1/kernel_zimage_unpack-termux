...
if [ -z "$GZIP_OFFSET_START" ]; then
    echo "GZip signature not found. Unable to extract data."
    exit 1
fi
...
if [ ! -s image.tmp ]; then
    echo "Error: Unable to extract gzip-compressed data."
    rm -f image.tmp
    exit 1
fi
...
if [ "$GZIP_ARCHIVE_SIZE" -eq 0 ]; then
    echo "Error: Failed to determine compressed data size."
    exit 1
fi
